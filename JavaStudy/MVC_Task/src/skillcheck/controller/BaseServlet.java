...
public abstract class BaseServlet extends HttpServlet {
	...
	protected boolean validateLogin(final HttpServletRequest request) throws IOException {
        ...
        try {
        	...
        	em.setEmpId(reqEmpId);
        	responseBean = ems.getEmployeeData(ExecuteCase.FIND_BY_EMPID,em);
            // 最初の1件を取得
            resEmployeeBean = responseBean.getEmplyeeBeanList().stream().findFirst().orElse(null);
            if (Objects.nonNull(resEmployeeBean)) {
                // パスワードチェック
            	final String empHashPassword = PasswordHashUtil.getSafetyPassword(resEmployeeBean.getPassword(),resEmployeeBean.getEmpId());
                final String hashPassword = PasswordHashUtil.getSafetyPassword(reqPassword, reqEmpId);
                if (empHashPassword.equals(hashPassword)) {
                    // ログイン成功
            ...
            this.executeSetExceptionInfo(e, ConstMessage.EXCEPTION_NULL1, 0);
